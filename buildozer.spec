...
from kivy.logger import Logger as KivyLogger # Using Kivy's logger for mobile compatibility
...
HAAR_CASCADE_FILENAME: str = "haarcascade_frontalface_default.xml" # Just the filename now
...
# Use Kivy's Logger for consistent logging across desktop and mobile
def Logger(message: str) -> None:
    KivyLogger.info(f"FaceApp: {message}")
...
class FaceAppBackend:
    def __init__(self):
        # Use Kivy's user_data_dir for writable storage on Android
        # This path is typically /data/data/org.kivy.faceapp/files on Android
        self.app_data_dir = App.get_running_app().user_data_dir
        self.known_faces_dir: str = str(Path(self.app_data_dir) / "known_faces")
        ensure_dir(self.known_faces_dir)
        Logger(f"[INFO] Known faces directory set to: {self.known_faces_dir}")

        # Path to Haar cascade classifier (bundled with the app)
        # On Android, files added with android.add_src are usually in the app's root directory
        self.haar_cascade_path = str(Path(App.get_running_app().directory) / HAAR_CASCADE_FILENAME)
        Logger(f"[INFO] Attempting to load Haar cascade from: {self.haar_cascade_path}")

        self.face_cascade = cv2.CascadeClassifier(self.haar_cascade_path)
        if self.face_cascade.empty():
            Logger(f"[WARN] Failed to load Haar cascade from '{self.haar_cascade_path}'. Attempting fallback to OpenCV data path.")
            fallback_path = cv2.data.haarcascades + HAAR_CASCADE_FILENAME
            self.face_cascade = cv2.CascadeClassifier(fallback_path)
            if self.face_cascade.empty():
                error_msg = (f"[ERROR] Failed to load Haar cascade classifier from both "
                             f"'{self.haar_cascade_path}' and '{fallback_path}'. "
                             f"Please ensure '{HAAR_CASCADE_FILENAME}' is present and accessible "
                             f"in your project folder and added to buildozer.spec's android.add_src.")
                Logger(error_msg)
                raise RuntimeError(error_msg)
            else:
                Logger(f"[INFO] Successfully loaded Haar cascade from fallback path: '{fallback_path}'.")
        else:
            Logger(f"[INFO] Successfully loaded Haar cascade from: '{self.haar_cascade_path}'.")
    ...
    def _train_recognizer(self):
        ...
        ensure_dir(self.known_faces_dir) # Ensure directory exists before listing
...
# This needs to be done AFTER Kivy App is running to get user_data_dir
face_app_backend = None # Initialize as None, will be set in FlaskKivyApp.on_start
...
def process_frame_endpoint():
    # Ensure backend is initialized before processing
    if face_app_backend is None:
        return jsonify({"status": "error", "message": "Backend not initialized. Please start Kivy app first."}), 500
...
class FlaskKivyApp(App):
    ...
    def on_start(self):
        ...
        Initialize the FaceAppBackend here to ensure App.user_data_dir is available.
        """
        global face_app_backend
        if face_app_backend is None:
            face_app_backend = FaceAppBackend()
            Logger("[INFO] FaceAppBackend initialized successfully.")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'runserver':
        ...
        # Note: The FaceAppBackend will NOT be initialized in this subprocess,
        # as it's meant to be managed by the main Kivy app process.
        # This Flask app will only handle API requests.
